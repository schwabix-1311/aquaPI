...
Die umfangreicheren Seiten und Layouts auf das neue SFC-Format überführt (rein strukturell, wie geplant ohne Wiederholung der inhaltlichen Erweiterungen aus Step 11-13). **(war zuvor Step 16)**
- `layouts/Default.vue`, `layouts/Auth.vue`, `pages/Home.vue`, `pages/Config.vue`, `pages/Settings.vue` als echte `.vue`-SFCs angelegt (Templates/Scripts 1:1 aus den bisherigen `.vue.js`-Dateien übernommen), lazy über `() => loadSfc(...)` in `router/index.js` geladen; alle 5 alten `.vue.js`-Dateien gelöscht.
- **Architektur-Erkenntnis**: Diese fünf Dateien sind reine Wrapper (Templates referenzieren global registrierte Tags wie `<aquapi-dashboard>`/`<aquapi-config>`/`<aquapi-nav-drawer>`); die zugehörigen `components/{dashboard,config,settings,app}/index.js`-Module (die diese Komponenten per Modul-Lade-Seiteneffekt registrieren) bleiben bewusst normale, weiterhin **eager** importierte `.js`-Dateien (jetzt eager in `router/index.js` statt wie zuvor in den Page-Dateien selbst) - ein `.vue`-`<script>`-Block kann solche `.js`-Dateien mit `import`/`export`-Syntax nicht direkt laden (siehe Step-19-Erkenntnis). `loadSfc.js`'s `moduleCache` wurde um virtuelle Einträge `app/EventBus`, `app/AquapiConfirmDialog` und `app/comps` erweitert, damit `Default.vue`s `<script>` diese pre-existing Module referenzieren kann.
- `components/app/registry.js` erweitert: `installGlobalComponents(app)` merkt sich jetzt die `app`-Instanz, sodass ein späterer `registerGlobalComponent(...)`-Aufruf (z.B. durch ein erst nach dem App-Mount importiertes Modul) sofort statt nur in eine beim Boot bereits geleerte Warteschlange registriert wird - schützt gegen zukünftige Lazy-Loading-Reihenfolgeprobleme.
- Vuex-Store-Module nutzen bereits seit Step 18 `createStore` (kein weiterer Umstellungsbedarf).
- Verifikation: Headless-Browser-Lauf (Puppeteer) gegen die laufende App mit echtem Login (volles Server-Redirect-Formular, nicht der clientseitige Auth-Platzhalter) bestätigt: Dashboard rendert nach Login, `/config` zeigt alle 13 echten Node-Boxen, `/settings` und `/about` rendern vollständig, kein neuer Konsolenfehler außer der bereits dokumentierten `vuedraggable`/`vue-masonry-css`-Einschränkung aus Step 18. `node --input-type=module --check` über alle geänderten `.js`-Dateien grün; kein Backend geändert, kein `pytest`-Lauf nötig.
- **Bekannte, nicht behobene Vorbedingung (außerhalb des Scopes dieses Schritts)**: Der clientseitige `store/modules/auth.js`-Login (`TODO: implement server side ...`) ist ein reiner `localStorage`/Vuex-Platzhalter ohne echten Bezug zur serverseitigen Flask-Login-Session; das Login-Formular führt bei Klick auf "Login" ein natives Formular-Submit aus (nicht das clientseitige `@submit.prevent`), wodurch tatsächlich die echte serverseitige Authentifizierung greift, der Nav-Drawer-Login/Logout-Icon-Zustand (abhängig vom Platzhalter-Vuex-Store) aber unabhängig davon bleibt und weiterhin "Login" anzeigt. Dies ist unverändertes Verhalten aus früheren Schritten, nicht durch diese Migration verursacht.

### ✓ Bugfix: Dashboard/Routing kaputt nach Vue-3-Migration, falsches Layout ohne Modal
Regression aus Step 18/20 (Detailplan: `.junie/plans/vue3-vuetify3-vuex4-migration.md`), root-caused via Headless-Browser-Session.
- Ursache: `AquapiDashboardConfigurator`s `v-navigation-drawer` steuerte seine Sichtbarkeit per `v-show`, das auf Vuetify-3-Multi-Root-Komponenten (Drawer + Scrim) wirkungslos ist - der Drawer blieb dadurch **dauerhaft sichtbar** und blockierte als Overlay die gesamte Seite inkl. Klicks (erklärt sowohl das falsche Layout als auch das kaputte Routing danach). Fix: Umstellung auf `:model-value`/`@update:model-value` + `location="right"`, analog zum bereits korrekt funktionierenden Muster in `AquapiNavDrawer`.
- `<masonry>` (Dashboard-Widget-Grid) und `<draggable>` (Konfigurator-Liste) waren wie in Step 18 dokumentiert Vue-2-only und blieben unregistriert. Fix: `<masonry>` durch ein eigenes, CSS-flex-basiertes Spalten-Layout ersetzt (`columnCount`/`columns`-Computed, resize-getrieben, mirrort die alten `{default: 3, 1264: 3, 960: 2, 600: 1}`-Breakpoints); `<draggable>` durch direktes `Sortable.create(...)` in `mounted()`/`unmounted()` ersetzt, committet über eine neue `reorderWidgets(from, to)`-Methode. Beide `<script>`-Tags (`vuedraggable.2.20.0.js`, `vue-masonry-css.1.0.3.js`) aus `spa.html.jinja2` entfernt.
- Zusätzlich gefunden und behoben: `AquapiNavDrawer.vue.js` verwendete an 4 Stellen die in Vuetify 3 entfernten `v-list-item-icon`/`v-list-item-content`-Tags (nicht auflösbar) - ersetzt durch `#prepend`-Slot + direkten `v-list-item-title`-Inhalt.
...
Self-Service-Passwort-Reset und Schutz vor Brute-Force-Logins ergänzt. **(war zuvor Step 18)**
- `aquaPi/db.py`: neue Tabellen `password_reset_tokens` (token, user_id, expires_at, used) und `login_attempts` (key, count, window_start, locked_until); `users`-Tabelle um nullable `email`-Spalte erweitert (automatische `ALTER TABLE`-Migration für bestehende DBs). Neue Funktionen `create_password_reset_token`/`get_password_reset_token`/`consume_password_reset_token` (Single-Use, TTL 30 Min.), `send_password_reset_email` (nutzt die bereits vorhandene Email-`notification_config`, kein neuer `DriverEmail`-Umweg nötig, da der bestehende an feste Hardware-Ports/Empfänger gebunden ist), sowie `is_login_locked_out`/`register_failed_login`/`clear_login_attempts` (5 Fehlversuche/15-Minuten-Fenster → 15 Minuten Sperre, Schwellwerte als Modulkonstanten `LOGIN_MAX_ATTEMPTS`/`LOGIN_ATTEMPT_WINDOW_MINUTES`/`LOGIN_LOCKOUT_MINUTES`); `create_user`/`set_user_email` unterstützen jetzt eine optionale E-Mail.
- `aquaPi/auth.py`: `/login` prüft vor der Passwortprüfung eine Sperre je Nutzername (Fallback: Remote-IP bei unbekanntem Nutzernamen) und registriert Fehlversuche/löscht sie bei Erfolg (Konstanten werden bei jedem Aufruf aus dem `db`-Modul gelesen, nicht als Funktions-Defaults gebunden, damit sie z.B. in Tests austauschbar bleiben). Neue Routen `GET/POST /reset-password` (Anfrage, immer identische generische Bestätigung unabhängig von Existenz/E-Mail des Kontos) und `GET/POST /reset-password/<token>` (Bestätigung, setzt neues Passwort). `/api/users/me`, `POST /api/users/`, `PUT /api/users/<id>` unterstützen jetzt ein optionales `email`-Feld.
- Neue Jinja-Templates `reset_password_request.html.jinja2`/`reset_password_confirm.html.jinja2` (im Stil von `login.html.jinja2`); "Passwort vergessen?"-Link im Login-Formular ergänzt.
...
Mehrsprachigkeit der SPA über die bereits eingebundene Vue-I18n-Bibliothek umsetzt. **(war zuvor Step 22)**
- **Erkenntnis bei der Analyse**: Die Extraktion bestehender Texte in Übersetzungsdateien war größtenteils bereits in früheren Steps (11-25) nebenbei erledigt worden - `i18n/locales/de.js`/`en.js` (verschachtelte JS-Objekte statt separater `.json`-Dateien, wie es die bereits existierende `i18n/index.js`-Struktur seit Step 18 vorgibt) deckten Dashboard/`/settings`/`/config`/`/users` schon fast vollständig ab. Eine gezielte Untersuchung fand nur noch 5 kleine verbliebene Lücken: den Hinweistext auf `/about` (`v-alert`), die Login-/Logout-Tooltips in der App-Bar (`Default.vue`), den "(alte) Settings"-Link sowie den Text der `AquapiDummy`-Komponente (tatsächlich als `view_bottom`-Slot auf `/settings` sichtbar, kein totes Testrelikt) - alle vier wurden auf neue i18n-Schlüssel (`pages.about.hintPlaceholder`, `pages.login.label`/`pages.logout.label` wiederverwendet, `misc.legacySettingsLink`, `misc.dummyComponentText`) umgestellt.
- **Sprachumschalter**: neuer `locale`-State + `setLocale`-Action in `store/modules/ui.js` (persistiert in `localStorage` unter `aquapi.locale`, analog zum bestehenden Dark-Mode-Muster); `main.js` liest den gespeicherten Wert beim Boot (Fallback: der bisherige `navigator.language`-Default aus `i18n/index.js`) und setzt `i18n.global.locale.value` direkt; neue `setLocale(locale)`-Root-Methode. `loadSfc.js`s `moduleCache` um einen virtuellen `app/i18n`-Eintrag erweitert, damit `.vue`-SFCs (z.B. `Default.vue`) die `i18n`-Instanz referenzieren können.
- UI: neuer Sprachumschalter-Menü-Button (Icon `mdi-translate`) in der App-Bar von `Default.vue`, zeigt beide verfügbaren Sprachen (`availableLocales`-Computed aus `i18n.global.messages.value`) mit aktiver Markierung (`currentLocale`-Computed).
- Verifikation: Headless-Browser-Lauf (Puppeteer) mit echtem Login bestätigt: Klick auf den Sprachumschalter übersetzt Navigation/Menü sofort vollständig (z.B. "Einstellungen"/"Konfiguration"/"Benutzer"/"Über aquaPi"/"Anmelden" → "Settings"/"Configuration"/"Users"/"About"/"Login"), die Wahl bleibt nach Reload dank `localStorage` erhalten, und alle 4 nachträglich gefundenen Lücken (About-Hinweis, Dummy-Komponente, Login-Tooltip, Legacy-Settings-Link) zeigen nach dem Wechsel korrekt die englischen Texte - keine Konsolenfehler, keine fehlenden Übersetzungsschlüssel. Alle geänderten `.js`-Dateien und die `<script>`-Blöcke der geänderten `.vue`-Dateien wurden per `node --input-type=module --check` geprüft; kein Backend geändert, kein `pytest`-Lauf nötig.
...
- **Erkenntnis bei der Analyse**: Der Light/Dark-Theme-Umschalter samt `localStorage`-Persistenz (`ui/setDarkMode`, `$root.toggleDarkMode`) existierte bereits vollständig seit Step 18 (inkl. des dort behobenen `$vuetify.theme.global`-API-Fixes) - hier war nichts mehr zu tun.
- Responsive-Check von Dashboard, `/settings`, `/config`, `/users` und `/about` per Headless-Browser (Puppeteer) bei 375px Viewport-Breite (iPhone-SE-Größe) durchgeführt: für jede Seite `document.documentElement.scrollWidth` gegen `window.innerWidth` verglichen und alle DOM-Elemente identifiziert, die breiter als der Viewport sind.
- Gefundene und behobene Regression: `AquapiPageHeading`s `<v-toolbar-title>` (genutzt von allen vier Kernseiten) nutzt Vuetify 3s `v-toolbar-title__placeholder`, das standardmäßig einzeilig ist (`overflow: hidden` + `text-overflow: ellipsis`) - längere deutsche Überschriften wie "Einstellungen"/"Konfiguration"/"Benutzerverwaltung" wurden dadurch zu "Einstellun…"/"Konfigura…"/"Benutzerv…" abgeschnitten. Fix in `app.css`: `h1.v-toolbar-title .v-toolbar-title__placeholder { white-space: normal; overflow: visible; text-overflow: unset; }`.
- Keine echte horizontale Seiten-Overflow gefunden: das Dashboard-Spaltenlayout ist bereits seit dem Step-18/20-Bugfix responsiv (`columnCount`-Computed reduziert bei < 960px/600px Breite auf 2/1 Spalten), der `/config`-Graph-Canvas sitzt in einem eigenen `overflow: auto`-Wrapper (`.config-canvas-wrapper`), und die Benutzertabelle (`v-data-table`) auf `/users` ist innerhalb ihres eigenen `.v-table__wrapper` horizontal wischbar (`overflow-x: auto`, `scrollWidth` 425px > `clientWidth` 303px bei 375px Viewport) statt hart abgeschnitten - ein gängiges, akzeptables Vuetify-Verhalten für schmale Tabellen mit vielen Spalten.
- Verifikation: Headless-Browser-Lauf (Puppeteer, 375×667 Viewport) mit echtem Test-Admin-Login bestätigt für `/`, `/settings`, `/config`, `/users`: kein `document.documentElement.scrollWidth > window.innerWidth`, keine Konsolenfehler; Screenshots vor/nach dem CSS-Fix bestätigen visuell, dass die Seitenüberschriften jetzt vollständig lesbar sind statt mit "…" abgeschnitten. Alle geänderten Dateien (`app.css`) sind reines CSS, kein `node --check`/`pytest`-Lauf nötig; Test-Account wieder gelöscht.

### ✓ Step 28: Alarm-Eskalation und erweiterte Sensor-Historie
Bestehende Alarm- und Zeitreihen-Funktionalität um Eskalation, Kalibrier-Historie und Export erweitert. **(war zuvor Step 24)**
- `aquaPi/db.py`: `user_notification_prefs` um `escalation_channel`/`escalation_after_minutes` ergänzt (automatische `ALTER TABLE`-Migration für bestehende DBs); `set_user_notification_pref()`, `list_user_notification_prefs()` und `get_prefs_for_alert()` erweitert.
- `aquaPi/machineroom/alert_nodes.py`: `Alert` trackt jetzt `_alert_since` (wann der aktuelle Alarm-Zustand begann) und `_escalated_users` (verhindert Mehrfach-Eskalation pro aktiver Episode); neue Methode `_check_escalation()` prüft bei **jeder** eingehenden Nachricht (unabhängig vom `repeat`/`any_change`-Throttling der Primär-Benachrichtigung), ob ein Nutzer einen abweichenden Eskalationskanal konfiguriert hat und die Alarmdauer dessen `escalation_after_minutes` erreicht/überschritten hat - dann wird zusätzlich (mit `[ESKALATION]`-Präfix) über `_notify_one()` benachrichtigt. `_alert_since`/`_escalated_users` werden zurückgesetzt, sobald der Alarm sich legt.
- `aquaPi/auth.py`: `PUT /api/notifications/prefs/<alert_node_id>` akzeptiert jetzt zusätzlich `escalation_channel`/`escalation_after_minutes` (validiert).
- Kalibrier-Historie: `aquaPi/machineroom/hist_nodes.py` bekam `log_calibration_event()`/`get_calibration_log()` (schreiben/lesen einer `calibration_log`-Tabelle in QuestDB, degradiert wie der Rest des Moduls graceful ohne QuestDB); `aquaPi/api.py`s `PUT /api/nodes/<id>/settings` erkennt Änderungen an `offset`/`factor` eines `ScaleAux`-Node (z.B. pH-Kalibrierung) und protokolliert sie automatisch; neue Route `GET /api/nodes/<id>/calibration-log`.
- Neue Route `GET /api/history/<id>/export` (Query-Parameter `format=json|csv`, Default `json`, sowie `start`/`step` wie bei `/api/history/<id>`) liefert eine Zeitreihe als CSV-Download oder JSON; protokolliert einen `export_history`-Audit-Log-Eintrag.
- Neue Testdateien `tests/test_alert_escalation.py` (10 Tests: DB-Migration/-Validierung, Eskalation nach konfigurierter Dauer, Reset nach Alarm-Ende, keine Doppel-Eskalation bei identischem Kanal, deaktivierte Eskalation) und `tests/test_history_export.py` (17 Tests: Export-Route inkl. Zugriffsschutz/Formate/Audit-Log, Kalibrier-Log-Route, `log_calibration_event`/`get_calibration_log` inkl. QuestDB-Fehlertoleranz, Kalibrier-Logging-Hook in der Settings-Route).
- Verifikation: `tests/test_alert_escalation.py` (10/10 grün) und `tests/test_history_export.py` (17/17 grün); Regressionschecks `tests/test_notifications.py` (16/16), `tests/test_health.py` (8/8), `tests/test_config_apply.py` (9/9) weiterhin grün; alle geänderten `.py`-Dateien per `py_compile` kompiliert. Hinweis: die Test-Fixtures erzwingen bewusst den In-Memory-Zeitreihen-Fallback (`QUEST_DB` gemockt), damit die Tests deterministisch sind, unabhängig davon, ob in der jeweiligen Umgebung eine echte QuestDB-Instanz erreichbar ist.
...
- **Erkenntnis bei der Analyse**: `tests/` mit 17 Testdateien und diversen lokalen `_io_registry`/`app`/`client`-Fixtures existierte bereits aus früheren Steps (6-28); dieser Schritt ergänzt eine gemeinsame, wiederverwendbare Basis, ohne die bestehenden Dateien anzufassen (lokale Fixtures überschreiben gleichnamige aus `conftest.py` weiterhin wie gewohnt).
- Neue `tests/conftest.py`: session-weite `io_registry`-Fixture (autouse, ruft `create_io_registry()` einmal auf), `users_db_path`/`topo_db_path`-Fixtures (temporäre SQLite-Pfade je Test via `tmp_path`), sowie `minimal_app`/`minimal_client` (schlanke Flask-Testing-App nur mit dem `auth`-Blueprint plus einem `spa.spa`-Stub, für Tests, die nicht den vollen `MachineRoom` brauchen).
- `requirements-dev.txt` (neu, referenziert `requirements.txt`) mit `pytest`/`pytest-mock`; `pytest.ini` (neu) mit `testpaths = tests` und Registrierung des Markers `questdb` (für Tests, die eine echte erreichbare QuestDB-Instanz voraussetzen); README um einen "Running the automated tests"-Abschnitt ergänzt (`pip install -r requirements-dev.txt && pytest`, `-m "not questdb"` zum Überspringen).
- Neue Beispieltestdatei `tests/test_msg_bus.py` (8 Tests für `machineroom/msg_bus.py`: Node-Registrierung/-Deregistrierung via `plugin()`/`pullout()`, doppelte ID abgelehnt, Nachrichtenweiterleitung an lauschende vs. nicht-lauschende Nodes, Wildcard-`receives='*'`, Rollenfilterung via `get_nodes(roles=...)`). Dabei bestätigt: `BusNode.listen()` schickt einem neu angemeldeten Listener automatisch den aktuellen Wert der Quelle einmalig nach (MsgHello-Mechanismus) - vorhandenes, korrektes Verhalten, in den Tests entsprechend berücksichtigt.
- Verifikation: `tests/test_msg_bus.py` (8/8 grün); Regressionschecks `tests/test_db.py` + `tests/test_msg_bus.py` (16/16), `tests/test_health.py` + `tests/test_node_crud_api.py` (32/32) weiterhin grün, bestätigt dass die neue `conftest.py` bestehende Tests nicht beeinflusst; `python3 -m py_compile` für `tests/conftest.py`/`tests/test_msg_bus.py` erfolgreich.
...
- **Erkenntnis bei der Analyse**: Tests für `aquaPi/db.py` (Schema-Erstellung, `topo.pickle`-Migration, Node-Rekonstruktion), Auth/Rollen (Login-Erfolg/-Fehlschlag, `@roles_required` für alle 3 Rollen) und Notification-Prefs (Kanal-Zuordnung je User/Alert) existierten bereits vollständig aus früheren Steps (`test_db.py`, `test_auth.py`, `test_auth_db.py`, `test_notifications.py`) - hier war nur noch `alert_nodes.py` selbst ungetestet.
- Neue Testdatei `tests/test_alert_nodes.py` (18 Tests): `AlertAbove`/`AlertBelow`-Schwellwertlogik, `AlertThreshold`-Dauer-Logik, Alert-Text-Generierung, sowie der `Alert`-Node selbst (Namens-Fix-Regressionstest für Commit `ab064bd`, Erstauslösung/Erholung, Repeat-Throttling, mehrere Bedingungen gleichzeitig, Port-Setter-Verhalten).
- **Echter Bug gefunden und behoben**: `AlertThreshold._check()` überschrieb `self._starttime` bei **jeder** erneuten Nachricht mit dem aktuellen `monotonic()`-Wert, solange die Bedingung erfüllt blieb - dadurch konnte die `duration`-Prüfung (`now >= self._starttime + duration*60`) bei `duration > 0` **niemals** auslösen, da `_starttime` immer auf `now` zurückgesetzt wurde. Fix in `aquaPi/machineroom/alert_nodes.py`: `_starttime` wird jetzt nur noch beim **ersten** Eintritt in den aktiven Zustand gesetzt (`if self._starttime is None: ...`), nicht mehr bei jeder Wiederholung.
- Als Nebenfund zwei veraltete Assertions in `tests/test_step10_integration.py` (`test_two_users_have_separated_dashboards_and_alert_prefs`) korrigiert, die noch das Vor-Step-28-Format von `/api/notifications/prefs` (ohne `escalation_channel`/`escalation_after_minutes`) erwarteten und dadurch fehlschlugen (bereits vor dieser Sitzung defekt, nicht durch die aktuelle Änderung verursacht).
- Verifikation: `tests/test_alert_nodes.py` (18/18 grün); Regressionschecks `tests/test_alert_escalation.py` + `tests/test_api_nodes.py` (16/16 grün, bestätigt dass der Timing-Fix die Eskalationslogik aus Step 28 nicht bricht) sowie `tests/test_step10_integration.py` (5/5 grün nach Korrektur der veralteten Assertions); `python3 -m py_compile` für `alert_nodes.py`/`test_alert_nodes.py` erfolgreich. Kein voller Suite-Lauf, gemäß vereinbartem Testprotokoll (gezielte Tests der betroffenen Module).

### ✓ Bugfix: Login-Konsolidierung sowie Dashboard-/Settings-Korrekturen (Detailplan: `.junie/plans/vue3-vuetify3-vuex4-migration.md`)
Eine neue, vom User gemeldete Korrekturrunde zu drei Bereichen (Login, Dashboard, Einstellungen) umgesetzt und per Headless-Browser-Session mit echter, geseedeten 13-Widget-Konfiguration verifiziert.
- **Login**: Von den drei bisherigen Login-Oberflächen (native Flask-Seite, SPA-Vollseiten-Route `/login`, Nav-Drawer-Dialog) bleibt nur noch der Nav-Drawer-Dialog übrig; `store/modules/auth.js` führt jetzt echte `fetch`-Requests gegen `/login`/`/logout` aus (Backend content-negotiiert via `X-Requested-With`-Header zwischen JSON und HTML/Redirect) und liest den echten Nutzer über `/api/users/me` aus; die native Login-Seite bleibt als technischer Fallback (u.a. `login_manager.login_view`-Ziel für `@login_required`) erhalten, ist aber aus der SPA nicht mehr verlinkt.
- **Dashboard**: Footer klebt jetzt zuverlässig an der unteren Fensterkante (auch bei überlaufendem Inhalt), das "nichts konfiguriert"-Hinweisfeld verschwindet korrekt sobald Widgets sichtbar sind, es werden bei ausreichender Breite wieder 3 statt 2 Spalten gerendert (jetzt per `ResizeObserver` am tatsächlichen Container statt an `window.innerWidth` gemessen, zusätzlich auf die Anzahl sichtbarer Widgets begrenzt), das Ausblenden eines Widgets lässt keine Spalte mehr komplett leer werden, der Konfigurator-Drawer sitzt bündig unter der Toolbar (fehlendes `app`/`dark` ergänzt), und Sub-Controls in den Karten sind jetzt auf allen Verschachtelungsebenen als echtes, klappbares `v-expansion-panels` (Vuetify-3-Tags `v-expansion-panel-title`/`-text` statt der alten Vuetify-2-Tags) umgesetzt.
- **Einstellungen**: Navigation zu `/settings` über die SPA funktioniert jetzt beim ersten Klick (Ursache: die `settings`-Route mischte als einzige Route einen lazy geladenen und einen synchron importierten `view_bottom`-Eintrag - dieser wurde entfernt), Slider rendern in Primärblau (`color="primary"`), einzelne Einstellungs-Widgets haben jetzt vertikalen Abstand (`mb-4`), und die orange "Dummy"-Demo-Karte ist nicht mehr in eine echte Seite eingebunden (Komponente selbst bleibt als wiederverwendbares Test-Fixture erhalten).
- **Zwei zusätzlich bei der Verifikation gefundene und behobene Bugs** (nicht Teil der ursprünglichen Diagnose): (1) das Entfernen der `AquapiDummy`-Einbindung aus `router/index.js` hatte versehentlich auch den einzigen Import von `components/app/index.js` entfernt, wodurch `AquapiPageHeading` (u.a. der "Dashboard konfigurieren"-Button) app-weit nicht mehr registriert wurde - durch einen expliziten Seiteneffekt-Import behoben; (2) `AquapiDashboardWidget`s `alert`-Computed prüfte nur, ob der Schlüssel `alert` existiert, crashte aber bei echten Daten mit `alert: null` (z.B. Node `heizen`) und fror dadurch die Dashboard-Reaktivität ein - Guard auf `this.node.alert == null` korrigiert. Außerdem stellte sich heraus, dass das bloße Entfernen des inline `max-height: 100vh` auf `v-main` allein nicht reichte, um den Footer zu fixieren: Vuetify 3 rendert keinen `.v-main__wrap`-Div mehr, daher wurde `app.css` um eine auf `100vh` begrenzte `.v-application__wrap` (mit `overflow: hidden`) sowie ein intern scrollendes `.v-main` (`overflow-y: auto`) ergänzt.
- Verifikation: gezielte Backend-Tests `tests/test_auth.py` (73/73 grün, inkl. 5 neuer Tests für die JSON-Login/Logout-Content-Negotiation) sowie Regressionschecks `tests/test_node_crud_api.py` + `tests/test_config_apply.py` (33/33 grün); Headless-Browser-Session (Puppeteer) mit einem temporären Test-Account und einer aus den echten 13 Nodes geseedeten Dashboard-Konfiguration bestätigt alle Punkte oben inkl. echtem Server-Login/-Logout (`/api/users/me` liefert nach Logout `401`), 3-Spalten-Layout ohne leere Spalte nach Ausblenden eines Widgets, funktionierendes Akkordeon, bündigen Konfigurator-Drawer, SPA-Navigation zu `/settings` ohne Reload, blaue Slider und entfernte Dummy-Karte - keine Konsolenfehler. Test-Account nach Verifikation wieder gelöscht, echte Accounts unangetastet.