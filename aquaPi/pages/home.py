...
def home():
    ...
    if not bus.dash_tiles:
        ...
        bus.dash_tiles = [{'name': 'Steuerung ' + n.name,
                           'comp': n.__class__.__name__,
                           'id': n.id,
                           'vis': int(True)}
                          for n in bus.get_nodes(BusRole.CTRL)]

        bus.dash_tiles += [{'name': 'Eingang ' + n.name,
                            'comp': n.__class__.__name__,
                            'id': n.id,
                            'vis': int(False)}
                           for n in bus.get_nodes(BusRole.IN_ENDP)]

        bus.dash_tiles += [{'name': 'Ausgang ' + n.name,
                            'comp': n.__class__.__name__,
                            'id': n.id,
                            'vis': int(False)}
                           for n in bus.get_nodes(BusRole.OUT_ENDP)]

        bus.dash_tiles += [{'name': 'Verknüpfung ' + n.name,
                            'comp': n.__class__.__name__,
                            'id': n.id,
                            'vis': int(False)}
                           for n in bus.get_nodes(BusRole.AUX)]

        bus.dash_tiles += [{'name': 'Diagramm ' + n.name,
                            'comp': n.__class__.__name__,
                            'id': n.id,
                            'vis': int(False)}
